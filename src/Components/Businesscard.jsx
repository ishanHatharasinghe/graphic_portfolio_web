// src/Components/BusinessCardsSection.jsx
    ...
    >
      ...
      <div className="relative max-w-7xl mx-auto px-6 py-24 md:py-28">
        ...
        >
          ...
          <p className="mt-6 max-w-2xl text-[#E7DFD6]/60">
            Clean, print‑ready business card designs with strong hierarchy,
            color, and brand presence.
          </p>
        </div>
        ...
        {/* No "See more" — extend morePosts to enable */}
        {morePosts.length > 0 && (
          <div className="mt-10 md:mt-14 flex justify-center">
            {/* See-more toggle area */}
          </div>
        )}
      </div>
      ...
      {overlayOpen && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Business card viewer"
          ref={overlayRef}
                ...
                <div className="grid grid-cols-3 sm:grid-cols-6 md:grid-cols-9 gap-2">
                  {allPosts.map((p, idx) => (
                    <button
                      key={`bc-thumb-${idx}`}
                      onClick={() => select(idx)}
                      className={`relative overflow-hidden rounded-lg aspect-[7/4] group transition ${
                        idx === activeIndex
                          ? "ring-2 ring-[#B08B57]"
